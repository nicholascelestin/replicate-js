...
test.serial('fetches details for the hello world model directly', async t => {
    ...
    t.log(`Fetched details for model ${modelName}, most recent version: ${mostRecentModelVersion}`)
    t.assert(mostRecentModelVersion.length > 0);
})

test.serial('starts a hello world prediction directly', async t => {
    const response = await replicate.startPrediction(mostRecentModelVersion, {text: "directly"});
    predictionId = response.id;
    t.log(`Started prediction for ${mostRecentModelVersion}, prediction id: ${predictionId}`)
...
test.serial('gets a hello world prediction directly', async t => {
    ...
    t.log(`Got details for prediction ${predictionId}, status: ${response.status}`)
