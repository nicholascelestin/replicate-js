...
export default class HeadlessBrowser {
  ...
  async serveJavascriptFile(fileName, filePath = null) {
    ...
    this.addRequestInterceptor(fileName, {
  ...
  addRequestInterceptor(urlSearchString, newResponse) {
    this.page.on("request", (req) => {
      if (req.url().includes(urlSearchString)) return req.respond(newResponse);
      req.continue();
    });
  }

  onConsoleLog(callback) {
    this.page.on("console", (consoleObj) => callback(consoleObj.text(), consoleObj.type()));
  }
  ...
  async runScript(fnToRun, context) {
    ...
    if (context) {
      ...
      serializedFunction = new Function(
        "context",
        `const fn = ${fnToRun.toString()}; return fn(window['context'])`
      );
    } else {
  ...
  async #exposeContextToBrowser(context) {
    ...
    for (const [key, val] of Object.entries(context)) {
      ...
      await this.page.evaluate(
        (safeFnName, key) => {
          window["context"][key] = window[safeFnName];
        },
        safeFnName,
        key
      );
